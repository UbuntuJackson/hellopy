...
class Keyboard:
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        if event.type == pygame.KEYDOWN:
            ...
            self.keys[event.key].end_time = time.time()
            self.keys[event.key].end_frame = pygame.time.get_ticks()

    def __getitem__(self, key: int) -> KKey:
        return self.key(key)

    def get_pressed(self) -> List[KKey]:
        return [k for k in self.keys.values() if k.pressed]

    def key(self, key: int) -> Optional[KKey]:
    ...
    def hold_time(self, key: int) -> Optional[float]:
        return time.time() - k.start_time if (k := self.key(key)) else None
    ...
    def time_since_release(self, key: int) -> Optional[float]:
        return time.time() - t if (t := self.release_time(key)) else None
